...
searchUsers :: (MonadIO m, MonadError e m, MonadReader r m, AsHttpError e, HasHttpCfg r, HasAuth0 r, FromJSON a, FromJSON b) => Query -> m [Profile' a b]
searchUsers q = do
    let path = "api/v2/users?search_engine=v2&q=" ++ (B.unpack . urlEncode False . B.pack $ show q)
    httpJSON =<< a0Req GET path NoRequestData

-- |Get a user by ID
getUser :: (MonadIO m, MonadError e m, MonadReader r m, AsHttpError e, HasHttpCfg r, HasAuth0 r, FromJSON a, FromJSON b) => String -> m (Profile' a b)
...
setEmail :: (MonadIO m, MonadError e m, MonadReader r m, AsHttpError e, HasHttpCfg r, HasAuth0 r, FromJSON a, FromJSON b) => String -> String -> m (Profile' a b)
...
setPhone :: (MonadIO m, MonadError e m, MonadReader r m, AsHttpError e, HasHttpCfg r, HasAuth0 r, FromJSON a, FromJSON b) => String -> String -> m (Profile' a b)
